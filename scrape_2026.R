...
while (TRUE) {
  ...
  # So pretend to be a browser using HTTP/1.1, which is simpler and doesn't look
  ...
  leaderboard <- map(1:length(data$data$player), function(player) {
    ...
    tibble(
      ...
      mutate(
        ...
        across(matches("_under$|^R|total_score"), ~ as.integer(.x))
      )
  }) |>
    list_rbind() |>
  ...
  tryCatch(
    write_sheet(
      data = leaderboard,
      ss = "https://docs.google.com/spreadsheets/d/1HfQffRhzhoucAZnjJF2Df9W2FOscOKOwcj2eVsnX9PY/edit?gid=0#gid=0",
      sheet = "leaderboard"
    ),
    error = function(e) {
